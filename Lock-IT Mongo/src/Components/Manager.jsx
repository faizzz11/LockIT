...
const Manager = () => {
  ...
  const savePass = async () => {
    if (
      ...
      // Only delete if we're editing an existing entry
      if (Form.id) {
  ...
  const EditPass = (id) => {
    ...
    const itemToEdit = PassArr.filter(i => i.id === id)[0];
    // Make sure we preserve the same ID for the edit
    setForm({ ...itemToEdit, id: id });
    setPassArr(PassArr.filter((item) => item.id !== id));
  };
      ...
      <div className="px-5 pt-14 mb-25">
        <div className="manager bg-purple-50 mycontainer rounded-md max-w-6xl mx-auto p-4 md:p-6 lg:p-8">
          ...
          <div className="passwords overflow-x-auto">
            <h2 className="font-bold font-[McLaren] text-xl sm:text-2xl py-3">
